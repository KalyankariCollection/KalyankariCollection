---
title: Wiki
icon: ri:diamond-line
index: false
---

# 🌟 Welcome to the Kalyankari Collection

Om Shanti! Welcome to Kalyankari Collection, a collection of curated resources for elevating the standard of Godly Services (Seva).

Browse by category in the sidebar, or scroll down to learn a little more about what this place is and how to make the most of it.

---

## What Is This?

> *"Kalyankari" (कल्याणकारी) — meaning One who does good for all.*

This collection was built to serve those who serve others. Whether you're looking for tools for productivity, creativity, privacy, learning, or anything in between — this wiki brings the best of the web into one trusted directory, organised and maintained with care.

Everything here is freely accessible, community-vetted, and arranged into clear categories so you can find what you need.

---

## 💡 Before You Start Browsing

**We strongly recommend installing an adblocker** before visiting any site in this collection. It will make your experience safer, faster, and completely ad-free:

- 🦊 **Firefox users** → Install [uBlock Origin](https://ublockorigin.com/)
- 🌐 **Chrome users** → Install **uBlock Origin Lite** (available in the Chrome Web Store)

It only takes a minute and makes a world of difference.

---

## 📣 Join Our Telegram Group

The Kalyankari Collection also has a **Telegram group** where we share updates, new additions, and discuss all things related to the collection. Come say hello!

👉 **[t.me/KalyankariCollection](https://t.me/KalyankariCollection)**

---

## 🤝 Want to Contribute?

This wiki grows through the kindness of people like you. If you have a great resource to share, found a broken link, or want to report something unsafe — please reach out:

- 💬 **Telegram**: [@KalyankariAngel](https://t.me/KalyankariAngel)
- 📧 **Email**: [KalyankariAngel@gmail.com](mailto:KalyankariAngel@gmail.com)

Every suggestion is welcome and appreciated. Sharing is an act of Seva too. 🙏

---

*Use the sidebar to explore the categories. Happy browsing!*
